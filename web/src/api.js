...
// ---------------------------------------------------------------------------
const S3_PART_SIZE = 50 * 1024 * 1024 // 50 MB per part — fewer round-trips = much faster
const PARALLEL_PARTS = 3 // upload 3 chunks simultaneously per file

function uploadS3PartDirect(url, chunk, timeoutMs, onProgress) {
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest()
    xhr.open('PUT', url)
...
export async function uploadFileS3(jobId, file, onProgress, onRetryStatus) {
  ...
  if (presign.method === 'PUT') {
    ...
    for (let i = 0; i < presign.parts.length; i++) {
      if (!completedParts[i]) queue.push(i)
    }

    // Upload with PARALLEL_PARTS concurrent workers — direct to S3
    const workers = Array.from({ length: Math.min(PARALLEL_PARTS, queue.length) }, async () => {
      while (queue.length > 0) {
        ...
        const etag = await withRetry(() => {
          partProgress[i] = 0
          const chunk = file.slice(start, end)
          return uploadS3PartDirect(partInfo.url, chunk, 300000, (loaded) => {
